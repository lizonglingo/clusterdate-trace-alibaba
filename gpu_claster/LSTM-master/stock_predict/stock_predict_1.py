...
#——————————————————导入数据——————————————————————
f = open('D:\Coding\WorkPlace\Python\clusterdate-trace-gpu2020\LSTM-master\stock_predict\dataset_1.csv')
...
def prediction():
    ...
    saver = tf.train.Saver(tf.global_variables())
    with tf.Session() as sess:
        saver.restore(sess, 'D:\Coding\WorkPlace\Python\clusterdate-trace-gpu2020\LSTM-master\stock_predict\\test_model_save\modle.ckpt')#参数恢复
        # I run the code in windows 10,so use  'model_save1\\modle.ckpt'
        # if you run it in Linux,please use  'model_save1/modle.ckpt'
